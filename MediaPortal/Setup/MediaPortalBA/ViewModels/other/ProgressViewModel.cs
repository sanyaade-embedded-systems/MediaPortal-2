...
namespace MediaPortal.InstallerUI.ViewModels.other
{
  public class ProgressViewModel : PropertyNotifyBase
  {
    ...
    public ProgressViewModel(RootViewModel root)
    {
      ...
      MpBootstrapperApplication.Model.Bootstrapper.ExecuteMsiMessage += this.ExecuteMsiMessage;
      MpBootstrapperApplication.Model.Bootstrapper.ExecuteProgress += this.ApplyExecuteProgress;
      MpBootstrapperApplication.Model.Bootstrapper.PlanBegin += this.PlanBegin;
      MpBootstrapperApplication.Model.Bootstrapper.PlanPackageComplete += this.PlanPackageComplete;
      MpBootstrapperApplication.Model.Bootstrapper.Progress += this.ApplyProgress;
      MpBootstrapperApplication.Model.Bootstrapper.CacheAcquireProgress += this.CacheAcquireProgress;
      MpBootstrapperApplication.Model.Bootstrapper.CacheComplete += this.CacheComplete;
    }
    ...
    private void CacheAcquireProgress(object sender, CacheAcquireProgressEventArgs e)
    {
      lock (this)
      {
        ...
        this.Progress = (this.cacheProgress + this.executeProgress) / (LaunchAction.Layout == MpBootstrapperApplication.Model.Command.Action ? 1 : 2);
        e.Result = this.root.Canceled ? Result.Cancel : Result.Ok;
      }
    }
    ...
    private void ApplyExecuteProgress(object sender, ExecuteProgressEventArgs e)
    {
      lock (this)
      {
        ...
        if (MpBootstrapperApplication.Model.Command.Display == Display.Embedded)
        {
          MpBootstrapperApplication.Model.Engine.SendEmbeddedProgress(e.ProgressPercentage, this.Progress);
        }
